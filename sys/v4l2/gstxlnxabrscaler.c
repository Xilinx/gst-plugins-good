...
gst_xlnx_abr_scaler_open (GstXlnxABRScaler * self)
{
  ...
  for (idx = 0; idx < g_list_length (self->srcpads); idx++) {
    ...
    if (!gst_v4l2_object_open (srcpad->v4l2output))
      goto failure;
    ...
    GstXlnxABRScalerPad * srcpad, GstCaps * incaps, GstCaps * outcaps)
{
  ...
  if (!gst_v4l2_object_set_crop (srcpad->v4l2output))
    goto failed;

  if (!gst_v4l2_object_set_crop (srcpad->v4l2capture))
    goto failed;
  ...
  for (idx = (g_list_length (self->srcpads) - 1); idx >= 0; idx--) {
    ...
    if (!gst_buffer_pool_is_active (out_pool)) {
      ...
      gint min =
          ...
          0 ? GST_V4L2_MIN_BUFFERS (srcpad->v4l2output) : srcpad->
          v4l2output->min_buffers;
