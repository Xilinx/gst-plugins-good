...
gst_v4l2_object_try_import (GstV4l2Object * obj, GstBuffer * buffer)
{
  ...
  if (!vmeta && obj->need_video_meta) {
    GST_DEBUG_OBJECT (obj->element, "Downstream buffer uses standard "
  ...
  if (vmeta) {
    ...
    if (need_fmt_update) {
      ...
      if (V4L2_TYPE_IS_MULTIPLANAR (obj->type)) {
        ...
        GST_DEBUG_OBJECT (obj->element, "Wanted strides:");
        ...
        for (i = 0; i < obj->n_v4l2_planes; i++) {
          ...
          GST_DEBUG_OBJECT (obj->element, "    [%u] %i", i, wanted_stride[i]);
        }
      } else {
        gint stride = vmeta->stride[0];

        GST_DEBUG_OBJECT (obj->element, "Wanted stride: %i", stride);
  ...
  if (n_mem != 1 && n_mem != obj->n_v4l2_planes) {
    GST_DEBUG_OBJECT (obj->element, "Can only import %i memory, "
  ...
  if (obj->mode == GST_V4L2_IO_DMABUF_IMPORT) {
    ...
    for (i = 0; i < n_mem; i++) {
      ...
      if (!gst_is_dmabuf_memory (mem)) {
        GST_DEBUG_OBJECT (obj->element, "Cannot import non-DMABuf memory.");
        return FALSE;
      }
    }
  }
