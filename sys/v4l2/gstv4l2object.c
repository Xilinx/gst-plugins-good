...
validate_video_meta_struct (GstV4l2Object * obj, const GstStructure * s)
{
  ...
  for (i = 0; i < gst_structure_n_fields (s); i++) {
    ...
    if (!g_str_equal (name, "single-allocation")
        && !g_str_equal (name, "padding-top")
    ...
    const GstStructure * s, GstCaps * caps, guint buffer_size)
{
  ...
  gst_structure_get_uint (s, "padding-top", &alig.padding_top);
  gst_structure_get_uint (s, "padding-bottom", &alig.padding_bottom);
  gst_structure_get_uint (s, "padding-left", &alig.padding_left);
  gst_structure_get_uint (s, "padding-right", &alig.padding_right);

  /* TODO: handle "single-allocation" field */

  if (alig.padding_top || alig.padding_bottom || alig.padding_left ||
      alig.padding_right) {
    GST_DEBUG_OBJECT (obj->dbg_obj,
        "Upstream requested padding (top: %d bottom: %d left: %d right: %d)",
        alig.padding_top, alig.padding_bottom, alig.padding_left,
        alig.padding_right);
  }
