...
#define XLNX_ROI_QUARK \
	g_quark_from_static_string("gst-xlnx-roi-info")

GST_DEBUG_CATEGORY_STATIC (gst_xlnx_roi_debug);
#define GST_CAT_DEFAULT gst_xlnx_roi_debug
...
#define gst_xlnx_roi_parent_class parent_class
G_DEFINE_ABSTRACT_TYPE (GstXlnxRoi, gst_xlnx_roi, GST_TYPE_BASE_TRANSFORM);

static void
gst_xlnx_roi_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  GstXlnxRoi *self = GST_XLNX_ROI (object);
  ...
  switch (prop_id) {
    ...
    case PROP_IMPORT_BUFFER_ALIG:
      ...
      break;
    case PROP_RELATIVE_QP:
      self->relative_qp = g_value_get_int (value);
      break;
...
static void
gst_xlnx_roi_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_IMPORT_BUFFER_ALIG:
      ...
      break;
    case PROP_RELATIVE_QP:
      g_value_set_int (value, self->relative_qp);
      break;
...
static gboolean
gst_xlnx_roi_open (GstXlnxRoi * self)
{
...
static void
gst_xlnx_roi_close (GstXlnxRoi * self)
{
...
static gboolean
gst_xlnx_roi_stop (GstBaseTransform * trans)
{
  GstXlnxRoi *self = GST_XLNX_ROI (trans);
...
static gboolean
gst_xlnx_roi_set_caps (GstBaseTransform * trans, GstCaps * incaps,
    GstCaps * tmpoutcaps)
{
  GstV4l2Error error = GST_V4L2_ERROR_INIT;
  GstXlnxRoi *self = GST_XLNX_ROI (trans);
  GstCaps *outcaps = gst_caps_new_simple ("video/x-raw",
      "width", G_TYPE_INT, DPU_INPUT_WIDTH,
      "height", G_TYPE_INT, DPU_INPUT_HEIGHT,
      "format", G_TYPE_STRING, DPU_INPUT_FORMAT,
...
static gboolean
gst_xlnx_roi_query (GstBaseTransform * trans, GstPadDirection direction,
    GstQuery * query)
{
  GstXlnxRoi *self = GST_XLNX_ROI (trans);
  gboolean ret = TRUE;
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_ALLOCATION:{
      GST_BASE_TRANSFORM_CLASS (parent_class)->propose_allocation (trans, NULL,
          query);
      break;
    }
    case GST_QUERY_CAPS:{
...
static gboolean
gst_xlnx_roi_decide_allocation (GstBaseTransform * trans, GstQuery * query)
{
  GstXlnxRoi *self = GST_XLNX_ROI (trans);
  gboolean ret = FALSE;
...
static gboolean
gst_xlnx_roi_propose_allocation (GstBaseTransform * trans,
...
static GstCaps *
gst_xlnx_roi_caps_remove_format_info (GstCaps * caps)
{
  GstStructure *st;
  GstCapsFeatures *f;
  gint i, n;
...
static GstCaps *
gst_xlnx_roi_transform_caps (GstBaseTransform * btrans,
    GstPadDirection direction, GstCaps * caps, GstCaps * filter)
{
  ...
  tmp = gst_xlnx_roi_caps_remove_format_info (caps);
...
static GstCaps *
gst_xlnx_roi_fixate_caps (GstBaseTransform * trans,
...
gst_xlnx_roi_prepare_output_buffer (GstBaseTransform * trans,
    GstBuffer * inbuf, GstBuffer ** outbuf)
{
  ...
  GstStructure *str = gst_caps_get_structure (self->incaps, 0);

  if (!gst_structure_get_int (str, "width", &width)) {
    GST_ERROR_OBJECT (self, "Error to get the width");
    goto beach;
  }

  if (!gst_structure_get_int (str, "height", &height)) {
    GST_ERROR_OBJECT (self, "Error to get the height");
    goto beach;
  }
  GstBuffer *tmpbuf = gst_buffer_new ();
  gst_buffer_replace_all_memory (tmpbuf, gst_buffer_get_memory (inbuf, 0));
  GST_DEBUG_OBJECT (self, "Queue input buffer");

  ret = gst_v4l2_buffer_pool_process (GST_V4L2_BUFFER_POOL (pool), &tmpbuf);
  ...
  do {
    ...
    ret = gst_buffer_pool_acquire_buffer (pool, &temp, NULL);
    g_object_unref (pool);
    ...
    ret = gst_v4l2_buffer_pool_process (GST_V4L2_BUFFER_POOL (pool), &temp);
  ...
  if (bclass->copy_metadata) {
    if (!bclass->copy_metadata (trans, inbuf, temp)) {
  ...
  GstMapInfo map;
  gst_buffer_map (temp, &map, GST_MAP_READWRITE);
  roi_region_data.actual_width = width;
  roi_region_data.actual_height = height;
  dpu_run_task (&roi_region_data, (unsigned char *) map.data);

  if (roi_region_data.num) {
    GstStructure *s;
    GstBuffer *roi;
    guint ts = 123456789;
    guint offset = 2;
    gint uint_size = sizeof (unsigned int);
    guint sei_data_size = roi_region_data.num * sizeof (GstRoiCoordinates) + 8;
    gchar *data = (gchar *) g_malloc (sei_data_size);

    memcpy (data, (char *) &roi_region_data.num, uint_size);
    memcpy (data + uint_size, (char *) &ts, uint_size);

    for (gint i = 0; i < roi_region_data.num; i++) {
      memcpy (data + (offset * uint_size),
          (char *) &(roi_region_data.roi_coordinates[i].xmin), uint_size);
      offset++;
      memcpy (data + (offset * uint_size),
          (char *) &(roi_region_data.roi_coordinates[i].ymin), uint_size);
      offset++;
      memcpy (data + (offset * uint_size),
          (char *) &(roi_region_data.roi_coordinates[i].width), uint_size);
      offset++;
      memcpy (data + (offset * uint_size),
          (char *) &(roi_region_data.roi_coordinates[i].height), uint_size);
      offset++;
    }

    GstPad *pad = GST_BASE_TRANSFORM_SRC_PAD (trans);
    /* Should be actual data */
    gchar *temp_data = g_malloc0 (sei_data_size);
    memcpy (temp_data, (char *) data, sei_data_size);
    roi = gst_buffer_new_wrapped (temp_data, sei_data_size);

    /* free SEI data memory */
    g_free (data);

    s = gst_structure_new ("omx-alg/insert-suffix-sei",
        "payload-type", G_TYPE_UINT, 77, "payload", GST_TYPE_BUFFER, roi, NULL);
    send_downstream_event (pad, s);
    gst_buffer_unref (roi);
  } else {
    GstVideoRegionOfInterestMeta *meta_all;
    meta_all =
        gst_buffer_add_video_region_of_interest_meta (inbuf, "dummy", 0, 0, 0,
        0);
    g_assert (meta_all);
    gst_video_region_of_interest_meta_add_param (meta_all,
        gst_structure_new ("roi/omx-alg", "quality", G_TYPE_STRING, "low",
            NULL));
  }

  GST_DEBUG_OBJECT (self, "xlnxroi :: roi count %u\n", roi_region_data.num);
  roi_align_data (&roi_region_data);

  GstVideoRegionOfInterestMeta **meta =
      g_malloc0 (roi_region_data.num * sizeof (GstVideoRegionOfInterestMeta *));

  for (int i = 0; i < roi_region_data.num; i++) {
    meta[i] =
        gst_buffer_add_video_region_of_interest_meta (inbuf, "face",
    ...
    g_assert (meta[i]);
    gst_video_region_of_interest_meta_add_param (meta[i],
        gst_structure_new ("roi-by-value/omx-alg", "delta-qp", G_TYPE_INT,
            self->relative_qp, NULL));
    GST_DEBUG_OBJECT (self,
        "xlnxroi :: roi no, xmin, ymin, width, height %u::%u::%u::%u::%u\n",
        (i + 1), roi_region_data.roi_coordinates[i].xmin,
        roi_region_data.roi_coordinates[i].ymin,
        roi_region_data.roi_coordinates[i].width,
        roi_region_data.roi_coordinates[i].height);
    GST_DEBUG_OBJECT (self, "Defining relative qp value %d", self->relative_qp);
  }

  g_free (meta);
  gst_buffer_unmap (temp, &map);
  g_free (roi_region_data.roi_coordinates);
...
static GstFlowReturn
gst_xlnx_roi_transform (GstBaseTransform * trans, GstBuffer * inbuf,
    GstBuffer * outbuf)
{
  /* Nothing to do */
  return GST_FLOW_OK;
...
static gboolean
gst_xlnx_roi_sink_event (GstBaseTransform * trans, GstEvent * event)
{
  GstXlnxRoi *self = GST_XLNX_ROI (trans);
  gboolean ret;
...
gst_xlnx_roi_change_state (GstElement * element, GstStateChange transition)
{
  GstXlnxRoi *self = GST_XLNX_ROI (element);
  GstStateChangeReturn ret;
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      densebox_init ();
      if (!gst_xlnx_roi_open (self))
...
static void
gst_xlnx_roi_dispose (GObject * object)
{
  GstXlnxRoi *self = GST_XLNX_ROI (object);
...
static void
gst_xlnx_roi_finalize (GObject * object)
{
  GstXlnxRoi *self = GST_XLNX_ROI (object);
...
static void
gst_xlnx_roi_init (GstXlnxRoi * self)
{
...
static void
gst_xlnx_roi_subinstance_init (GTypeInstance * instance, gpointer g_class)
{
  GstXlnxRoiClass *klass = GST_XLNX_ROI_CLASS (g_class);
  GstXlnxRoi *self = GST_XLNX_ROI (instance);
...
static void
gst_xlnx_roi_class_init (GstXlnxRoiClass * klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (gst_xlnx_roi_debug, "xlnxroi", 0, "V4L2 Converter");

  gst_element_class_set_static_metadata (element_class,
      "V4L2 Video ROI detector",
      "Filter/Converter/Video/Scaler",
      "Transform streams via V4L2 API and send roi region",
      "Nicolas Dufresne <nicolas.dufresne@collabora.com>, "
      "Saket Kumar Bafna <saketb@xilinx.com>");

  gobject_class->dispose = GST_DEBUG_FUNCPTR (gst_xlnx_roi_dispose);
  gobject_class->finalize = GST_DEBUG_FUNCPTR (gst_xlnx_roi_finalize);
  gobject_class->set_property = GST_DEBUG_FUNCPTR (gst_xlnx_roi_set_property);
  gobject_class->get_property = GST_DEBUG_FUNCPTR (gst_xlnx_roi_get_property);

  base_transform_class->stop = GST_DEBUG_FUNCPTR (gst_xlnx_roi_stop);
  base_transform_class->set_caps = GST_DEBUG_FUNCPTR (gst_xlnx_roi_set_caps);
  base_transform_class->query = GST_DEBUG_FUNCPTR (gst_xlnx_roi_query);
  base_transform_class->sink_event =
      GST_DEBUG_FUNCPTR (gst_xlnx_roi_sink_event);
  base_transform_class->decide_allocation =
      GST_DEBUG_FUNCPTR (gst_xlnx_roi_decide_allocation);
  base_transform_class->propose_allocation =
      GST_DEBUG_FUNCPTR (gst_xlnx_roi_propose_allocation);
  base_transform_class->transform_caps =
      GST_DEBUG_FUNCPTR (gst_xlnx_roi_transform_caps);
  base_transform_class->fixate_caps =
      GST_DEBUG_FUNCPTR (gst_xlnx_roi_fixate_caps);
  base_transform_class->prepare_output_buffer =
      GST_DEBUG_FUNCPTR (gst_xlnx_roi_prepare_output_buffer);
  base_transform_class->transform = GST_DEBUG_FUNCPTR (gst_xlnx_roi_transform);

  element_class->change_state = GST_DEBUG_FUNCPTR (gst_xlnx_roi_change_state);
  ...
  g_object_class_install_property (gobject_class, PROP_RELATIVE_QP,
      g_param_spec_int ("relative-qp", "relative qp",
          "User can select relaative qp values for the roi region to tune the ROI region quality",
          -32, 31, -21, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}

static void
gst_xlnx_roi_subclass_init (gpointer g_class, gpointer data)
{
  GstXlnxRoiClass *klass = GST_XLNX_ROI_CLASS (g_class);
  GstElementClass *element_class = GST_ELEMENT_CLASS (g_class);
  GstXlnxRoiCData *cdata = data;
...
/* Probing functions */
gboolean
gst_xlnx_roi_is_transform (GstCaps * sink_caps, GstCaps * src_caps)
{
  gboolean ret = FALSE;
...
void
gst_xlnx_roi_register (GstPlugin * plugin, const gchar * basename,
    const gchar * device_path, GstCaps * sink_caps, GstCaps * src_caps)
{
  ...
  GType type, subtype;
  gchar *type_name;
  GstXlnxRoiCData *cdata;
  cdata = g_new0 (GstXlnxRoiCData, 1);
  cdata->device = g_strdup (device_path);
  cdata->sink_caps = gst_caps_ref (sink_caps);
  cdata->src_caps = gst_caps_ref (src_caps);

  type = gst_xlnx_roi_get_type ();
  g_type_query (type, &type_query);
  memset (&type_info, 0, sizeof (type_info));
  type_info.class_size = type_query.class_size;
  type_info.instance_size = type_query.instance_size;
  type_info.class_init = gst_xlnx_roi_subclass_init;
  type_info.class_data = cdata;
  type_info.instance_init = gst_xlnx_roi_subinstance_init;

  type_name = g_strdup_printf ("xlnxroi%sdetect", basename);
